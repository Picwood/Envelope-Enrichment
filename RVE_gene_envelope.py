...
for file in files:
    ...
    re_pos = re.compile(r'\s*\d*\,\s*(\S+\.?\d*)\,\s+(\S+\.?\d*)\,\s+(\S+\.?\d*)', re.VERBOSE)  # inpfile
    # re_pos = re.compile(r'\d*\s(\S+\.?\d*)\s(\S+\.?\d*)\s(\S+\.?\d*)\s0+?',re.VERBOSE) #mshfile
    ...
    filepath_inp = os.path.join(folder, subfolder_name, '%s-VER.inp' %(name))
    filepath_txt = os.path.join(folder, subfolder_name, '%s-VER.e2a' %(name))
